...
def progress_stats(df):
    ...
    Used to make a DataFrame which can then be used to make a Venn Diagram.
    ...
    df_ground_truths.loc['Seizure-Free', ('Lateralising Datapoints', 'Total')
                         ] = df.loc[df[post_op].notnull()].Lateralising.sum()
    df_ground_truths.loc['Seizure-Free', ('Localising Datapoints', 'Total')
                         ] = df.loc[df[post_op].notnull()].Localising.sum()

    df_ground_truths.loc['Concordant', ('Lateralising Datapoints', 'Total')
                         ] = df.loc[df[concordant].notnull()].Lateralising.sum()
    df_ground_truths.loc['Concordant', ('Localising Datapoints', 'Total')
                         ] = df.loc[df[concordant].notnull()].Localising.sum()

    df_ground_truths.loc['sEEG and|or ES', ('Lateralising Datapoints', 'Total')
                         ] = df.loc[df[sEEG_ES].notnull()].Lateralising.sum()
    df_ground_truths.loc['sEEG and|or ES', (
        'Localising Datapoints', 'Total')] = df.loc[df[sEEG_ES].notnull()].Localising.sum()

    df_ground_truths.loc['ES', ('Lateralising Datapoints', 'Total')
                         ] = df.loc[df[sEEG_ES] == 'ES'].Lateralising.sum()
    df_ground_truths.loc['ES', ('Localising Datapoints', 'Total')
                         ] = df.loc[df[sEEG_ES] == 'ES'].Localising.sum()
...
def progress_venn(df_ground_truths, method=None, plot=False):
    ...
    # set a tuple
    numbers = (sz_excl, conc_excl, sz_conc, sEEG_excl,
               sEEG_ES_sz, sEEG_ES_conc, all_three)
    a = [int(n) for n in numbers]
    numbers = tuple(a)

    # plot
    if plot:
        venn3(subsets=(numbers), set_labels=(
            'Seizure-Free', 'Concordant', 'sEEG/ES'))
